...
public:
    struct Configuration: public Thread::Configuration {
        Configuration(const Microsecond & p, const Microsecond & d = SAME, const Microsecond & cap = UNKNOWN, const Microsecond & act = NOW, const unsigned int n = INFINITE, const State & s = READY, unsigned int ss = STACK_SIZE)
        : Thread::Configuration(s, Criterion(p, d, cap), ss), period(p), deadline(d == SAME ? p : d), capacity(cap), activation(act), times(n) {}
